...
given pattern or not. The pattern is: I(d)*_SB(d)+ where I=[a-z] and d=[0-9]
*/
#include<stdio.h>
#include<stdlib.h>
#include<string.h>
...
int isMatched(char *str){
    ...
    for(i=0; i < strlen(str); i++)
    {
        ...
        else if(state == 1 && (str[i] >= '0' && str[i] <= '9')){
            state = 2;
        }
        else if((state == 2 || state == 3) && (str[i] >= '0' && str[i] <= '9') ){
            state = 3;
        }
        else if((state == 1 || state==3) && str[i] == '_' ){
            state = 4;
        }
        else if(state==4 && str[i] == 'S' ){
            state = 5;
        }
